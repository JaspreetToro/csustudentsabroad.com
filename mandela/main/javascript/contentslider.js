...
ContentSlider.buildpagination=function(sliderid){
	...
	{	
		...
		pcontent+='<div id="item'+i+'"  onClick=\"ContentSlider.turnpage(\''+sliderid+'\', '+i+'); return false\"   onmouseover=\"if(document.getElementById(\'link_item'+i+'\').className != \'selected\'){this.id=\'item'+i+'_selected\';document.getElementById(\'link_item'+i+'\').className = \'selected\';}\"   onmouseout=\"cookieval=getCookie(\''+sliderid+'\').split(\':\');if(parseInt(cookieval[1]) != '+i+'){this.id=\'item'+i+'\';document.getElementById(\'link_item'+i+'\').className = \'\';}\" class="feature_link"><div style="height: 7px; font-size: 1px;"></div><a href="#" id=\"link_item'+i+'\" onClick=\"ContentSlider.turnpage(\''+sliderid+'\', '+i+'); return false\">'+(slider.paginateText? slider.paginateText[i] : i+1)+'</a></div>'
	}
...
ContentSlider.turnpage=function(sliderid, thepage){
	...
	{ //For each DIV within slider
		...
		if(document.getElementById('item'+i+'_selected') != null)
			document.getElementById('item'+i+'_selected').id = 'item'+i;
	}

	paginatelinks[thepage].className="selected" //for selected DIV, set corresponding pagination link's class name
	
	document.getElementById('item'+thepage).id = 'item'+thepage+'_selected';
