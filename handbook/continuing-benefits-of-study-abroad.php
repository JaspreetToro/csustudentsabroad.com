<?php
	...
	$pageTitle = "Making the Study Abroad Experience at Home";
	...
	try {
        ...
        $conn = connect_db();
        $makingStudyAbroadExperienceLinkTemplates= array();    
		...
		foreach ($links as $key => $value) {			
            ...
            if (preg_match("/_w_text$/", $key)) {
    			$row->set("title", $value);
				$row->set("URL", $links[substr($key, 0, -5)]);
                $makingStudyAbroadExperienceLinkTemplates[] = $row;
            }
		}
		...
		$makingStudyAbroadExperienceLinkContents = Template::merge($makingStudyAbroadExperienceLinkTemplates);
		...
		$makingStudyAbroadExperienceList  = new Template("content/handbook/coming-home/making-study-abroad-experience-content.tpl");
		
		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$makingStudyAbroadExperienceList->set("country", "the ".$country['country_name']);
        }
		else {
			$makingStudyAbroadExperienceList->set("country", $country['country_name']);
		}
		
		$makingStudyAbroadExperienceList->set("nationality1", $country['nationality']);
		$makingStudyAbroadExperienceList->set("nationality_with_article", $country['nationality_with_article']);
		$makingStudyAbroadExperienceList->set("countryPrimaryLanguage", $country['primary_language']);
		$makingStudyAbroadExperienceList->set("linksToRemember", $makingStudyAbroadExperienceLinkContents);	
		...
		*/
		...
		$layout->set("content", $makingStudyAbroadExperienceList->output());
