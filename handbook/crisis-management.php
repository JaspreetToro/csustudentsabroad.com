<?php
	...
	try {
        ...
        $conn = connect_db();
        $crisisManagementLinkTemplates = array();    
		$links = query_all_links_table($country_name);
		
		foreach ($links as $key => $value) {			
			$row = new Template("content/handbook/health-and-safety/crisis-management-links.tpl");
			
			if (preg_match("/_s_text$/", $key)) {
    			$row->set("title", $value);
				$row->set("URL", $links[substr($key, 0, -5)]);
                $crisisManagementLinkTemplates[] = $row;
            }
		...
		$crisisManagementLinkContents = Template::merge($crisisManagementLinkTemplates);
		...
		if ($country_name == "United States") {
			$crisisManagementList  = new Template("content/handbook/health-and-safety/us-crisis-management-content.tpl");
		} else {
			$crisisManagementList  = new Template("content/handbook/health-and-safety/crisis-management-content.tpl");
		}
		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$crisisManagementList->set("country", "the ".$country['country_name']);
        }
		else {
			$crisisManagementList->set("country", $country['country_name']);
		}
		
        $crisisManagementList->set("country_link", $country_name);
		$crisisManagementList->set("countryPrimaryLanguage", $country['primary_language']);
		$crisisManagementList->set("linksToRemember", $crisisManagementLinkContents);	
		...
		*/
		...
		$layout->set("content", $crisisManagementList->output());
