<?php
	...
	$pageTitle = "Selecting the Right Program for You";
	...
	try {
        ...
        $conn = connect_db();
        $selectTheRightProgramLinkTemplates = array();    
		$links = query_all_links_table($country_name);
		
		foreach ($links as $key => $value) {
			$row = new Template("content/handbook/choosing-a-program/select-the-right-program-links.tpl");
			
			if (preg_match("/_f_text$/", $key)) {
    			$row->set("title", $value);
                $row->set("URL", $links[substr($key, 0, -5)]);
                $selectTheRightProgramLinkTemplates[] = $row;
            }
		...
		$selectTheRightProgramLinkContents = Template::merge($selectTheRightProgramLinkTemplates);
		...
		if ($country_name == "United States") {
			$selectTheRightProgramList  = new Template("content/handbook/choosing-a-program/us-select-the-right-program-content.tpl");
		} else {
			$selectTheRightProgramList  = new Template("content/handbook/choosing-a-program/select-the-right-program-content.tpl");
		}
		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$selectTheRightProgramList->set("country", "the ".$country['country_name']);
        }
		else {
			$selectTheRightProgramList->set("country", $country['country_name']);
		}
		
		$selectTheRightProgramList->set("countryPrimaryLanguage", $country['primary_language']);
		$selectTheRightProgramList->set("LevelsofImmersionA", $country['levels_of_immersion1']);
		$selectTheRightProgramList->set("LevelsofImmersionB", $country['levels_of_immersion2']);
		$selectTheRightProgramList->set("linksToRemember", $selectTheRightProgramLinkContents);	
		...
		*/
		...
		$layout->set("content", $selectTheRightProgramList->output());
