<?php
	...
	try {
		...
		foreach ($links as $key => $value) {
			$row = new Template("content/handbook/advice-for-parents-links.tpl");
			
            if (preg_match("/_d_text$/", $key)) {
    			$row->set("title", $value);
		...
		$adviceForParentsList  = new Template("content/handbook/advice-for-parents-content.tpl");
		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$adviceForParentsList->set("country", "the ".$country['country_name']);
			$adviceForParentsList->set("country_link", $country_name);
        }
		else {
			$adviceForParentsList->set("country", $country['country_name']);
			$adviceForParentsList->set("country_link", $country_name);
		}
		
		$adviceForParentsList->set("linksToRemember", $countryLinkContents);	
		...
		*/
		...
		$layout->set("content", $adviceForParentsList->output());
