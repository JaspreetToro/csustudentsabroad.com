<?php
	...
	$pageTitle = "Financing Study Abroad";
	...
	try {
        ...
        $conn = connect_db();
        $financingStudyAbroadProgramLinkTemplates = array();    
		$links = query_all_links_table($country_name);
		
		foreach ($links as $key => $value) {
			$row = new Template("content/handbook/choosing-a-program/financing-study-abroad-links.tpl");
			
			if (preg_match("/_h_text$/", $key)) {
    			$row->set("title", $value);
				$row->set("URL", $links[substr($key, 0, -5)]);
                $financingStudyAbroadProgramLinkTemplates[] = $row;
            }			
		}
		...
		$financingStudyAbroadProgramLinkContents = Template::merge($financingStudyAbroadProgramLinkTemplates);
		...
		if ($country_name == "United States") {
			$financingStudyAbroadProgramList  = new Template("content/handbook/choosing-a-program/us-financing-study-abroad-content.tpl");
		} else {
			$financingStudyAbroadProgramList  = new Template("content/handbook/choosing-a-program/financing-study-abroad-content.tpl");
		}
		...
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$financingStudyAbroadProgramList->set("country", "the ".$country['country_name']);
			$financingStudyAbroadProgramList->set("country_link", $country_name);
        }
		else {
			$financingStudyAbroadProgramList->set("country", $country['country_name']);
			$financingStudyAbroadProgramList->set("country_link", $country_name);
		}
		
		$financingStudyAbroadProgramList->set("nationality1", $country['nationality']);
		$financingStudyAbroadProgramList->set("countryCurrency", $country['currency']);
		$financingStudyAbroadProgramList->set("linksToRemember", $financingStudyAbroadProgramLinkContents);	
		...
		*/
		...
		$layout->set("content", $financingStudyAbroadProgramList->output());
