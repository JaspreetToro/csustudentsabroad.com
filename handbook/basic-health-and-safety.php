	...
	try {
		...
		foreach ($country_health_safety as $key => $value) {
            ...
            if (preg_match("/_type$/", $key)) {
				if(!$country_health_safety[substr($key, 0, -4).'description'] == ""){
		...
		if ($country_name == "United States") {
			$basicHealthAndSafetyList  = new Template("content/handbook/health-and-safety/us-basic-health-and-safety-content.tpl");
		} else {
		...
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$basicHealthAndSafetyList->set("country", "in the ".$country['country_name']);
			$basicHealthAndSafetyList->set("country_table", "the ".$country['country_name']);
			$basicHealthAndSafetyList->set("country_resources", $country['country_name']);
        }
		elseif($country_name == "General") {
			$basicHealthAndSafetyList->set("country", "Abroad");
			$basicHealthAndSafetyList->set("country_table", "Worldwide");
			$basicHealthAndSafetyList->set("country_resources", "General");
		}
		else {
			$basicHealthAndSafetyList->set("country", "in ".$country['country_name']);
			$basicHealthAndSafetyList->set("country_table", $country['country_name']);
			$basicHealthAndSafetyList->set("country_resources", $country['country_name']);
		}
