<?php
	...
	$pageTitle = "Medical Care and Insurance";
	...
	try {
        ...
        $conn = connect_db();
        $medicalCareAndInsuranceLinkTemplates = array();    
		$links = query_all_links_table($country_name);
		
		foreach ($links as $key => $value) {
			$row = new Template("content/handbook/health-and-safety/medical-care-and-insurance-links.tpl");
			
			if (preg_match("/_o_text$/", $key)) {
    			$row->set("title", $value);
                $row->set("URL", $links[substr($key, 0, -5)]);
                $medicalCareAndInsuranceLinkTemplates[] = $row;
            }	
		}
		...
		$medicalCareAndInsuranceLinkContents = Template::merge($medicalCareAndInsuranceLinkTemplates);
		...
		if ($country_name == "United States") {
			$medicalCareAndInsuranceList  = new Template("content/handbook/health-and-safety/us-medical-care-and-insurance-content.tpl");
		} else {
			$medicalCareAndInsuranceList  = new Template("content/handbook/health-and-safety/medical-care-and-insurance-content.tpl");
		}
		
		
		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$medicalCareAndInsuranceList->set("country", "the ".$country['country_name']);
        }
		else {
			$medicalCareAndInsuranceList->set("country", $country['country_name']);
		}
		
		$medicalCareAndInsuranceList->set("countryPrimaryLanguage", $country['primary_language']);
		$medicalCareAndInsuranceList->set("linksToRemember", $medicalCareAndInsuranceLinkContents);	
		...
		*/
		...
		$layout->set("content", $medicalCareAndInsuranceList->output());
