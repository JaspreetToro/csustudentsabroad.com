<?php
	...
	$pageTitle = "Communication While Abroad";
	/* 
	...
	try {
        ...
        $conn = connect_db();
        $communicationWhileAbroadLinkTemplates= array();    
		$links = query_all_links_table($country_name);
		
		foreach ($links as $key => $value) {
			$row = new Template("content/handbook/practical-information/method-of-communication-while-abroad-links.tpl");
			
            if (preg_match("/_l_text$/", $key)) {
    			$row->set("title", $value);
				$row->set("URL", $links[substr($key, 0, -5)]);
                $communicationWhileAbroadLinkTemplates[] = $row;
            }
		}
		...
		$communicationWhileAbroadLinkContents = Template::merge($communicationWhileAbroadLinkTemplates);
		...
		if ($country_name == "United States") {
			$communicationWhileAbroadList  = new Template("content/handbook/practical-information/us-method-of-communication-while-abroad-content.tpl");
		} else {
			$communicationWhileAbroadList  = new Template("content/handbook/practical-information/method-of-communication-while-abroad-content.tpl");
		}
		
		
		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$communicationWhileAbroadList->set("country", "the ".$country['country_name']);
        }
		else {
			$communicationWhileAbroadList->set("country", $country['country_name']);
		}
		
        $communicationWhileAbroadList->set("country_link", $country_name);        
		$communicationWhileAbroadList->set("nationality1", $country['nationality']);
		$communicationWhileAbroadList->set("nationality2", $country['nationality_with_article']);
		$communicationWhileAbroadList->set("linksToRemember", $communicationWhileAbroadLinkContents);	
		...
		*/
		...
		$layout->set("content", $communicationWhileAbroadList->output());
