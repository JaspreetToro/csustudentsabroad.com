<?php
	...
	try {
        ...
        $conn = connect_db();
        $packingLinkTemplates = array();    
		$links = query_all_links_table($country_name);
		
		foreach ($links as $key => $value) {
			$row = new Template("content/handbook/practical-information/packing-links.tpl");
			
			if (preg_match("/_m_text$/", $key)) {
    			$row->set("title", $value);
                $row->set("URL", $links[substr($key, 0, -5)]);
                $packingLinkTemplates[] = $row;
            }
		}
		...
		$packingLinkContents = Template::merge($packingLinkTemplates);
		...
		if ($country_name == "United States") {
			$packingList  = new Template("content/handbook/practical-information/us-packing-content.tpl");
		} else {
			$packingList  = new Template("content/handbook/practical-information/packing-content.tpl");
		}

		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$packingList->set("country", "the ".$country['country_name']);
        }
		else {
			$packingList->set("country", $country['country_name']);
		}
		
        $packingList->set("country_link", $country_name);
		$packingList->set("nationality1", $country['nationality']);
		$packingList->set("nationality2", $country['nationality_with_article']);
		$packingList->set("linksToRemember", $packingLinkContents);	
		...
		*/
		...
		$layout->set("content", $packingList->output());
