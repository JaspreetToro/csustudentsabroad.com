<?php
	...
	try {	
		/**
		 * Loop through the helpImages and creates a template for each one.
		 ...
		 * We save each template in the $helpImagesTemplates array.
		 */
        ...
        $images = query_all_images_table($country_name);
		
        /* Word to Know */
        // When add new words (use google doc)
        // sed -e 's/^/"/' file | sed -e 's/$/"/' | tr '\r\n' ',' > news		
		$englishWord = array(1 => "Emergency","Fire","Taxi","Police","Ambulance","Hospital","Computer","Post Office","Bank","Telephone",
					"Bathroom","Water","Hotel","Airport","Metro");
		$englishWord_c1 = array(1 => "Emergency","Fire","Taxi","Police","Ambulance","Hospital","Computer","Post Office","Bank","Telephone",
					"Bathroom","Water","Hotel","Airport","Metro");
		//$helpImagesTemplates = array(); 
		
		foreach ($englishWord as $translationKey => $english) {
			$row = new Template("content/help-images-links.tpl");
            $row->set("english", $english);
			$row->set("counter", 'image'.$translationKey);
			$helpImagesTemplates[] = $row;
		}
		
		foreach ($englishWord_c1 as $translationKey_c1 => $english_c1) {
			$row = new Template("content/help-images-card1.tpl");
            $row->set("english", $english_c1);
			$row->set("counter", 'image'.$translationKey_c1);
            $row->set("translation", $images['image' . $translationKey_c1]);
			 $row->set("translation1", $images['image1']);
			 $row->set("country1", strtolower($country_name));
			//print_r($row);
			$helpImages_c1_Templates[] = $row;
		}
		/**
		 * Merges all our helpImages' templates into a single variable.
		 * This will allow us to use it in the main template.
		 */
		$helpImagesContents = Template::merge($helpImagesTemplates);
		$helpImages_c1_Contents = Template::merge($helpImages_c1_Templates);
		
		/**
		 * Defines the main template and sets the helpImages' content.
		 */
		$country = query_all_country_table($country_name);
        if (empty($country)) throw new Exception("country does not exist"); 
		$helpImagesList  = new Template("content/help-images-content.tpl");
		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$helpImagesList->set("country", "the ".$country['country_name']);
        }
		else {
			$helpImagesList->set("country", $country['country_name']);
		}
		
		$helpImagesList->set("helpImagesTable", $helpImagesContents);	
		$helpImagesList->set("helpImages_c1_Table", $helpImages_c1_Contents);
		/**
		...
		*/
		...
		$layout->set("content", $helpImagesList->output());
