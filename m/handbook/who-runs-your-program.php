<?php
	...
	$pageTitle = "Who Runs your Program?";
	/* 
	...
	try {
        ...
        $conn = connect_db();
        $whoRunsYourProgramLinkTemplates = array();    
		$links = query_all_links_table($country_name);
        		
		foreach ($links as $key => $value) {
			$row = new Template("content/handbook/choosing-a-program/who-runs-your-program-links.tpl");
			
			if (preg_match("/_g_text$/", $key)) {
				$row->set("title", html_entity_decode($value));
                $row->set("URL", $links[substr($key, 0, -5)]);
                $whoRunsYourProgramLinkTemplates[] = $row;
            }
		...
		$whoRunsYourProgramLinkContents = Template::merge($whoRunsYourProgramLinkTemplates);
		...
		if ($country_name == "United States") {
			$whoRunsYourProgramList  = new Template("content/handbook/choosing-a-program/us-who-runs-your-program-content.tpl");
		} else {
			$whoRunsYourProgramList  = new Template("content/handbook/choosing-a-program/who-runs-your-program-content.tpl");
		}	
		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$whoRunsYourProgramList->set("country", "the ".$country['country_name']);
        }
		else {
			$whoRunsYourProgramList->set("country", $country['country_name']);
		}
		
		$whoRunsYourProgramList->set("nationality1", $country['nationality']);
		$whoRunsYourProgramList->set("nationality2", $country['nationality_with_article']);
		$whoRunsYourProgramList->set("linksToRemember", $whoRunsYourProgramLinkContents);	
		...
		*/
		...
		$layout->set("content", $whoRunsYourProgramList->output());
