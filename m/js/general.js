$(document).ready(function () {														
	...
	$('#goto_fullsite').click(function () {
		...
		//alert("first: " + first_directory);
        var full_site_directory = currenturl.substring((currenturl.indexOf("/", 0) + 1)) //delete the first directory
		//alert("full site dir: " + full_site_directory);
		if (first_directory === "" || first_directory === "m") {
			var full_site_url = "http://" + "studentsabroad.com/" + full_site_directory;
		} else {
			var full_site_url = "http://" + "studentsabroad.com/" + first_directory + "/" + full_site_directory;
		}
		//alert("full site url: " + full_site_url);
		var country_url = full_site_directory.substring(0, 16);
		var country_var = full_site_directory.substring((full_site_directory.indexOf("=", 0) + 1));
		//alert(country_url);
		//alert(country_var);
		if( full_site_directory === "student-handbook.php" || full_site_directory === "student-handbook.php#") {
			window.location = "http://studentsabroad.com/handbook/introduction.php?country=General";
		} else if (full_site_directory === "program-search.php" || full_site_directory === "program-search.php#") {
			window.location = "http://directory.studentsabroad.com/index.cfm?FuseAction=Programs.SimpleSearch";
		} else if (country_url === "country.php?name" ) {
			window.location = "http://studentsabroad.com/handbook/introduction.php?country=" + country_var; 	
		} else {
	...
	$('#shEmergencyCardForm').each(function () {							  	
			$(this).validate({
				groups: {
					dob: "birth_month birth_date birth_year"				
				},		
				rules: {
					full_name: {
						required: true
				...
				messages: {
					...
					program_name: {
						required: "Please enter your program name",
					},
					wishes: {
						required: "Please enter primary contact",
					},
					wishes_tel: {
						required: "Please enter primary contact phone",
					},
					family_contact: {
						required: "Please enter secondary contact",
					},
					family_tel: {
						required: "Please enter secondary contact phone",
					},
					insurance_company: {
					...
					emergency_phone: {
						required: "Please enter 24 hour emergency phone",
					},
					abroad_program_contact: {
						required: "Please enter abroad program contact",
					},
					abroad_program_tel: {
						required: "Please enter abroad program phone",	
					},
					home_campus_contact: {
						required: "Please enter home campus emergency contact",
					},
					home_campus_tel: {
						required: "Please enter home campus phone",
					},
					embassy_consulate: {
						required: "Please enter embassy/consulate",
					},
					embassy_consulate_tel: {
						required: "Please enter embassy/consulate phone",
					},
					equivalent_911_abroad: {
						required: "Please enter equivalent of 9-1-1 abroad phone",
					},					
					personal_email: {
				...
				errorPlacement: function(error, element) {
				  ...
				  if (n == "birth_year" || n == "birth_month" || n == "birth_year")
				    error.insertBefore(".citizenship");
				  else
				    error.insertAfter(element);
				}//,
				//submitHandler: function (form) { 
					//alert("submit");
					//form.submit(getResults('shEmergencyCardForm', 'printCard'));
					//return false;
					/*$('.sh_button').click( function(e) {
						e.preventDefault();
						$( '#myEmergencyCardForm2' ).submit(getResults2('myEmergencyCardForm2', 'step2'));
					});*/
				//}
			});	
		});
		//$('.hi_translation_wrapper').html($('.hi_translation_wrapper').html().replace('/','<br>'));
		$(".hi_translation_wrapper").css({wordWrap: "break-word", width: "130px" });
});
...
$(window).load(function () {
    ...
    /* table */
    $(".list-all table tr:even").addClass("even");
	$(".list-all table tr:odd").addClass("odd");
});
...
// required
function validateForm(formName, resultsType) {
	var str_results_type = resultsType;
	
	var str_full_name = document.forms[formName]["full_name"].value;	
	if(str_full_name.length < 1)
	{
		full_name.focus();
		return false;	
	}	
	
	var str_birth_month = document.forms[formName]["birth_month"].value;	
	if(str_birth_month.length < 1)
	{
		birth_month.focus();
		return false;	
	}	
	var str_birth_date = document.forms[formName]["birth_date"].value;
	if(str_birth_date.length < 1)
	{
		birth_date.focus();
		return false;	
	}	
	var str_birth_year = document.forms[formName]["birth_year"].value;
	if(str_birth_year.length < 1)
	{
		birth_year.focus();
		return false;	
	}	
	var str_citizenship = document.forms[formName]["citizenship"].value;
	if(str_citizenship.length < 1)
	{
		citizenship.focus();
		return false;	
	}	
	var str_passport_number = document.forms[formName]["passport_number"].value;
	if(str_passport_number.length < 1)
	{
	...
	if(str_family_contact.length < 1)
	{
...
function getResults(formName, resultsType) {
	...
	var file_path="success-emergency-card.php?";
