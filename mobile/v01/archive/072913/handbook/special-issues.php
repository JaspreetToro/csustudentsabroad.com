<?php
	...
	try {
        ...
        $conn = connect_db();
        $specialIssuesLinkTemplates = array();    
		$links = query_all_links_table($country_name);
        		
		foreach ($links as $key => $value) {
			$row = new Template("content/handbook/health-and-safety/special-issues-links.tpl");
			
			if (preg_match("/_r_text$/", $key)) {
    			$row->set("title", $value);
                $row->set("URL", $links[substr($key, 0, -5)]);
                $specialIssuesLinkTemplates[] = $row;
            }
		...
		$specialIssuesLinkContents = Template::merge($specialIssuesLinkTemplates);
		...
		if ($country_name == "United States") {
			$specialIssuesList  = new Template("content/handbook/health-and-safety/us-special-issues-content.tpl");
		} else {
			$specialIssuesList  = new Template("content/handbook/health-and-safety/special-issues-content.tpl");
		}
		
		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$specialIssuesList->set("country", "the ".$country['country_name']);
        }
		else {
			$specialIssuesList->set("country", $country['country_name']);
		}
		
		$specialIssuesList->set("countryPrimaryLanguage", $country['primary_language']);
		$specialIssuesList->set("linksToRemember", $specialIssuesLinkContents);	
		...
		*/
		...
		$layout->set("content", $specialIssuesList->output());
