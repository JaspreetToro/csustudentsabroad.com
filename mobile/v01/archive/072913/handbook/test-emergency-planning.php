<?php
	...
	try {
		...
		$emergencyPlanningContent  = new Template("content/emergency-planning-content.tpl");
		//$emergencyPlanningContent->set("country", $country['country_name']);
		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$emergencyPlanningContent->set("country", "the ".$country_name);
        }
		else {
			$emergencyPlanningContent->set("country", $country_name);
		}	
                /*$emergencyPlanningContent->set("nationality", $country['nationality']);
		$emergencyPlanningContent->set("nationality1", $country['nationality_with_article']);
		$emergencyPlanningContent->set("countryPrimaryLanguage", $country['primary_language']);
		$emergencyPlanningContent->set("language", $country['primary_language']);*/
		...
		$layout->set("title", "Emergency Planning");
		$layout->set("header", "Emergency Planning");
		...
		*/
		...
		$layout->set("content", $emergencyPlanningContent->output());
