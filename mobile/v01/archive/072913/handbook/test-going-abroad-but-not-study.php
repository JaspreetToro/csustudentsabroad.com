<?php
	...
	$pageTitle = "Going Abroad...but not Studying?";
	...
	try {
		...
		$templateContent  = new Template("content/going-abroad-but-not-study.tpl");
		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$templateContent->set("country", "the ".$country['country_name']);
        }
		else {
			$templateContent->set("country", $country['country_name']);
		}
		
		$templateContent->set("countryPrimaryLanguage", $country['primary_language']);
		...
		$layout->set("title", "Going Abroad...but not Studying?");
		$layout->set("header", "Going Abroad...but not Studying?");
		...
		*/
		...
		$layout->set("content", $templateContent->output());
