<?php
    ...
    class Template {
		...
		public function test() {
			...
			// check if some variables are not set. If not, hightlight it with red bold 
            $output = preg_replace("/(\\[@[^\\]\\[]+\\])/", "<strong><font color='red'>$1</font></strong>", $output);
			
			// check if old varible are not changed. If not, highlight with blue bold
			$output = preg_replace("/(#[^#]+#)/", "<strong><font color='blue'>$1</font></strong>", $output);
            return $output;		
		}
