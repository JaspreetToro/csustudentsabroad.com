<?php
	...
	$pageTitle = "Airport Safety, Duties and Customs";
	/* 
	...
	try {
        ...
        $conn = connect_db();
        $airportProceduresLinkTemplates = array();    
		$links = query_all_links_table($country_name);
		
		foreach ($links as $key => $value) {
			$row = new Template("content/handbook/coming-home/airport-safety-duties-custom-links.tpl");
			
            if (preg_match("/_u_text$/", $key)) {
    			$row->set("title", $value);
				$row->set("URL", $links[substr($key, 0, -5)]);
                $airportProceduresLinkTemplates[] = $row; 
            }
		}
		...
		$airportProceduresLinkContents = Template::merge($airportProceduresLinkTemplates);
		...
		if ($country_name == "United States") {
			$airportProceduresList  = new Template("content/handbook/coming-home/us-airport-safety-duties-custom-content.tpl");
		} else {
			$airportProceduresList  = new Template("content/handbook/coming-home/airport-safety-duties-custom-content.tpl");
		}
		...
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$airportProceduresList->set("country", "the ".$country['country_name']);
        }
		else {
			$airportProceduresList->set("country", $country['country_name']);
		}
		
		$airportProceduresList->set("nationality1", $country['nationality']);
		$airportProceduresList->set("countryPrimaryLanguage", $country['primary_language']);
		$airportProceduresList->set("linksToRemember", $airportProceduresLinkContents);	
		...
		*/
		...
		$layout->set("content", $airportProceduresList->output());
