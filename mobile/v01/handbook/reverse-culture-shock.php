<?php
	...
	$pageTitle = "Reverse Culture Shock";
	/* 
	...
	try {
        ...
        $conn = connect_db();
        $reverseCultureShockLinkTemplates = array();    
		$links = query_all_links_table($country_name);
		
		foreach ($links as $key => $value) {
			$row = new Template("content/handbook/coming-home/reverse-culture-shock-links.tpl");
			
			if (preg_match("/_v_text$/", $key)) {
    			$row->set("title", $value);
                $row->set("URL", $links[substr($key, 0, -5)]);
                $reverseCultureShockLinkTemplates[] = $row;
            }			
		}
		...
		$reverseCultureShockLinkContents = Template::merge($reverseCultureShockLinkTemplates);
		...
		$reverseCultureShockList  = new Template("content/handbook/coming-home/reverse-culture-shock-content.tpl");
		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$reverseCultureShockList->set("country", "the ".$country['country_name']);
        }
		else {
			$reverseCultureShockList->set("country", $country['country_name']);
		}
		
		$reverseCultureShockList->set("nationality1", $country['nationality']);
		$reverseCultureShockList->set("countryPrimaryLanguage", $country['primary_language']);
		$reverseCultureShockList->set("linksToRemember", $reverseCultureShockLinkContents);	
		...
		*/
		...
		$layout->set("content", $reverseCultureShockList->output());
