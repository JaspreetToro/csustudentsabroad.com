<?php
	...
	try {
        ...
        $conn = connect_db();
        $expectationsLinkTemplates = array();    
		$links = query_all_links_table($country_name);
				
		foreach ($links as $key => $value) {
			$row = new Template("content/handbook/practical-information/expectation-links.tpl");
			
			if (preg_match("/_n_text$/", $key)) {
    			$row->set("title", $value);
				$row->set("URL", $links[substr($key, 0, -5)]);
                $expectationsLinkTemplates[] = $row;
            }
		}
		...
		$expectationsLinkContents = Template::merge($expectationsLinkTemplates);
		...
		if ($country_name == "United States") {
			$expectationsList  = new Template("content/handbook/practical-information/us-expectation-content.tpl");
		} else {
			$expectationsList  = new Template("content/handbook/practical-information/expectation-content.tpl");
		}		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$expectationsList->set("country", "the ".$country['country_name']);
        }
		else {
			$expectationsList->set("country", $country['country_name']);
		}	
		
		$expectationsList->set("nationality1", $country['nationality']);
		$expectationsList->set("linksToRemember", $expectationsLinkContents);	
		...
		*/
		...
		$layout->set("content", $expectationsList->output());
