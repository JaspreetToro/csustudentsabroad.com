<?php
	...
	try {
		...
		if ($country_name == "United States") {
			$checkList  = new Template("content/us-checklist-content.tpl");
		} else {
			$checkList  = new Template("content/checklist-content.tpl");
		}		
		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$checkList->set("country", "in the ".$country['country_name']);
			$checkList->set("countryPrimaryLanguage", $country['primary_language']);
        } else if ($country_name == "General") {
			 $checkList->set("country", "abroad");
			 $checkList->set("countryPrimaryLanguage", "a specific language");
		} else {
			$checkList->set("country", "in ".$country['country_name']);
			$checkList->set("countryPrimaryLanguage", $country['primary_language']);
		}
		
		$checkList->set("nationality", $country['nationality']);
		$checkList->set("nationality1", $country['nationality_with_article']);	
		$checkList->set("language", $country['primary_language']);
		...
		$layout->set("title", "Checklist");
		$layout->set("header", "Checklist");
		...
		*/
		...
		$layout->set("content", $checkList->output());
