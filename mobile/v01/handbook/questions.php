<?php
	...
	try {
		...
		$questionsList  = new Template("content/questions-content.tpl");
		
		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$questionsList->set("country", "in the ".$country['country_name']);
			
        }
		elseif ($country_name == "General") {
			 $questionsList->set("country", "abroad");	
		} else {
			$questionsList->set("country", "in ".$country['country_name']);
		}
		
		$questionsList->set("nationality", $country['nationality']);
		$questionsList->set("nationality1", $country['nationality_with_article']);
		$questionsList->set("language", $country['primary_language']);
		...
		$layout->set("title", "Questions");
		$layout->set("header", "Questions");
		...
		*/
		...
		$layout->set("content", $questionsList->output());
