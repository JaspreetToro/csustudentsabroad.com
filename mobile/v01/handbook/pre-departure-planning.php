<?php
	...
	$pageTitle = "Pre-Departure Planning";
	/* 
	...
	try {
        ...
        $conn = connect_db();
        $preDeparturePlanningLinkTemplates = array();    
		$links = query_all_links_table($country_name);
		
		foreach ($links as $key => $value) {
			$row = new Template("content/handbook/practical-information/pre-departure-planning-links.tpl");			
			if (preg_match("/_j_text$/", $key)) {
    			$row->set("title", $value);
                $row->set("URL", $links[substr($key, 0, -5)]);
                $preDeparturePlanningLinkTemplates[] = $row;
            }
		...
		$preDeparturePlanningLinkContents = Template::merge($preDeparturePlanningLinkTemplates);
		...
		if ($country_name == "United States") {
			$preDeparturePlanningList  = new Template("content/handbook/practical-information/us-pre-departure-planning-content.tpl");
		} else {
			$preDeparturePlanningList  = new Template("content/handbook/practical-information/pre-departure-planning-content.tpl");
		}


		if ($country_name == "United States" || $country_name == "United Kingdom" || $country_name == "Czech Republic" || $country_name == "Dominican Republic") {
			$preDeparturePlanningList->set("country", "the ".$country['country_name']);
        }
		else {
			$preDeparturePlanningList->set("country", $country['country_name']);
		}
		
		$preDeparturePlanningList->set("nationality1", $country['nationality']);
		$preDeparturePlanningList->set("countryPrimaryLanguage", $country['primary_language']);
		$preDeparturePlanningList->set("linksToRemember", $preDeparturePlanningLinkContents);	
		...
		*/
		...
		$layout->set("content", $preDeparturePlanningList->output());
